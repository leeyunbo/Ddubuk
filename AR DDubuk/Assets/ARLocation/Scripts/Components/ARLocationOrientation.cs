...
public class ARLocationOrientation : MonoBehaviour
{
    ...
    [Tooltip("If true, only measure during the session's start-up. If false, update the heading every time we calculate an average value." +
             " Setting to false will give more accurate results but may cause some jittering in the scene object's.")]
    public bool measureOnlyAtStartup = true;                                
    ...
    /// </summary>
    [Tooltip("The smoothing factor. Zero means disabled. Values around 100 seem to give good results."), Range(0, 500)] // default = 70.0f
    public float smoothing = 85.0f;

    /// <summary>
    /// A custom offset to the device-calculated true north direction.
    /// </summary>
    [Tooltip("A custom offset to the device-calculated true north direction.")] // north direction 
    ...
    void Start()
    {
        ...
        target = Camera.main.transform.rotation.eulerAngles.y; //y축 기준 현재 오일러값을 얻는다.
        ...
        locationProvider.OnCompassUpdated((newHeading, lastHeading) =>
        {
            ...
            float currentCameraHeading = Camera.main.transform.rotation.eulerAngles.y;//현재 카메라가 향하고 있는방향
            float value = Utils.GetNormalizedDegrees(currentCameraHeading - ((float)trueHeading));// 0~ 360도 범위로 바꿔줌. ex) 760도면 760%360 =40 
